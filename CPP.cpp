...
List c_Prepare(S4 x,bool simplify,int qua,bool standardize){
  ...
  if(simplify){
    ...
    for(j=0;j<vars;j++){
      ...
      NumericMatrix AA=as<NumericMatrix>(TMP_res(0));//è la matrice
...
NumericMatrix c_ADA_F_WHEIGHT(List distances,int k, int vars, int ind, int schema, int weight_sys,
  ...
  for (clu=0;clu<k;clu++){
    sr=sum(DiM(_,clu));
    if (sr<1e-10){DiM(_,clu)=DiM(_,clu)+1e-10;}
    sr=sum(DiV(_,clu));
    if (sr<1e-10){DiV(_,clu)=DiV(_,clu)+1e-10;}
  }
...
// [[Rcpp::export]]
double c_WH_ADPT_FCMEANS_SSQ2(S4 x,NumericMatrix memb,double m,
                             NumericMatrix lambdas, S4 proto, double theta){
  int ind=((as<ListMatrix>(x.slot("M"))).nrow());
  int vars=((as<ListMatrix>(proto.slot("M"))).ncol());
  ListMatrix proto_cont=proto.slot("M");
  ...
  for (i=0;i<ind;i++){
    for (clu=0;clu<k;clu++){
      for (j=0;j<vars;j++){
        S4 o1=x_cont(i,j);
        S4 pro_clu=proto_cont(clu,j);
        List tmpd= c_Wass_Q_dist_2P(o1, pro_clu);
        double dM=tmpd["Dm"];
        double dV=tmpd["Dv"];
        SSQ=SSQ+(pow(memb(i,clu),m)*(pow(lambdas((j*2),clu),theta)*dM+
          pow(lambdas((j*2+1),clu),theta)*dV));          
...
// [[Rcpp::export]]
List c_WH_ADPT_KMEANS_TOTALSSQ2(S4 x, NumericMatrix memb, double m,
                               NumericMatrix lambdas,S4 proto){//theta how to use it? 
  int ind=((as<ListMatrix>(x.slot("M"))).nrow());
  int vars=((as<ListMatrix>(proto.slot("M"))).ncol());
  ListMatrix proto_cont=proto.slot("M");
  ...
  for (j=0;j<vars;j++){
    ...
    for(clu=0;clu<k;clu++){
      ...
      double locm=locprot.slot("m");
      BSQ_clu_m(j,clu)=sum(memb(_,clu))*lambdas((j*2),clu)*(GENPROTm-locm)*(GENPROTm-locm);
      List tmpd= c_Wass_Q_dist_2P(o, locprot);
      double tmpdv=tmpd["Dv"];
      BSQ_clu_d(j,clu)=sum(memb(_,clu))*lambdas((j*2+1),clu)*tmpdv;
  ...
  for (i=0;i<ind;i++){
    for (clu=0;clu<k;clu++){
      for (j=0;j<vars;j++){
        S4 o1=x_cont(i,j);
        S4 prog2=MEANS_cont(0,j);
        List tmpd= c_Wass_Q_dist_2P(o1, prog2);
        double dM=pow((MATMEA(i,j)-centersM(0,j)),2);
        double dV=tmpd["Dv"];
        TSQ2_clu_m(j,clu)=TSQ2_clu_m(j,clu)+(pow(memb(i,clu),m)*lambdas((j*2),clu)*dM);
        TSQ2_clu_d(j,clu)=TSQ2_clu_d(j,clu)+(pow(memb(i,clu),m)*lambdas((j*2+1),clu)*dV);
  ...
  //resu2["SEC"]=resu;
  //resu2["Wm"]=W_m;resu2["Wd"]=W_d;
//  resu2["TSQ_m"]=0;resu2["TSQ_v"]=TSQ_clu_d;
  resu2["TSQ2_m"]=TSQ2_clu_m;resu2["TSQ2_v"]=TSQ2_clu_d;
  return resu2;
}
...
// [[Rcpp::export]]
List c_ComputeFast_L2_SQ_WASS_DMAT2(S4 x,S4 prot){
  
  ListMatrix data_con=x.slot("M");
  ListMatrix prot_con=prot.slot("M");
  int ind=data_con.nrow();
  int k=prot_con.nrow();
  int vars=prot_con.ncol();
  ListOf<List> DistaF(vars);
  NumericMatrix CompDista(ind,k);
  ...
  for (j=0;j<vars;j++){
    ...
    for (clu=0;clu<k;clu++){
      ...
      for (i=0;i<ind;i++){
        S4 obj=data_con(i,j);
        //Rcout<<"yea\n";
        List tmpd=c_Wass_Q_dist_2P(obj, pro);
        //Rcout<<"yea2\n";
        Dista(i,clu)=tmpd["D"];
        DistM(i,clu)=tmpd["Dm"];
        DistV(i,clu)=tmpd["Dv"];
        CompDista(i,clu)=CompDista(i,clu)+Dista(i,clu);
      }
...
// [[Rcpp::export]]
NumericMatrix c_ComputeFast_L2_SQ_WASS_ADA_DMAT2(S4 x,
                                        S4 prot,
                                        NumericMatrix RW){
  ...
  for (j=0;j<vars;j++){
    ...
    for (clu=0;clu<k;clu++){
      ...
      for (i=0;i<ind;i++){
        ...
        DistM=tmpd["Dm"];
        DistV=tmpd["Dv"];
        CompDista(i,clu)=CompDista(i,clu)+
          RW(j*2,clu)*DistM+RW((j*2+1),clu)*DistV;
      }
...
// [[Rcpp::export]]
List c_Prepare3(S4 x,bool simplify,int qua,bool standardize){
  int ind=((as<ListMatrix>(x.slot("M"))).nrow());
  int vars=((as<ListMatrix>(x.slot("M"))).ncol());
  int i,j;
  
  NumericMatrix MQ(ind,vars);
  NumericMatrix SQ(ind,vars);
  List c(vars);
  List r(vars);
  List w(vars);
  ...
  for (j=0;j<vars;j++){
    ...
    MTMP=as<NumericMatrix>(MM[j]);
    int rows=MTMP.nrow();
    NumericMatrix Ct(rows-1,ind);
    NumericMatrix Rt(rows-1,ind);
    NumericVector pcum=MTMP(_,ind);
    NumericVector Wt=diff(pcum);
    ...
    for(i=0;i<ind;i++){
      ...
      MQ(i,j)=m*m;
      SQ(i,j)=s*s;
      NumericVector doms=MTMP(_,i);
      Ct(_,i)=(doms[Range(0,(rows-2))]+doms[Range(1,(rows-1))])*0.5;
      Rt(_,i)=diff(doms)/2.0;
    }
    c[j]=Ct;
    r[j]=Rt;
    w[j]=Wt;
...
// [[Rcpp::export]]
List c_ComputeFast_L2_SQ_WASS_DMAT_2(List C, List R, List W, S4 prot){
  
  NumericMatrix C0=C[0];
  int ind=(C0.ncol());
  ...
  for (j=0;j<vars;j++){
    ...
    NumericMatrix DistV(ind,k);
    NumericMatrix Dista(ind,k);
    NumericMatrix Ct=C[j];
    NumericMatrix Rt=R[j];
    NumericVector w=W[j];
    int rr=Ct.nrow()+1;
    for (clu=0;clu<k;clu++){
      S4 pro=prot_con(clu,j);
      ...
      for (i=0;i<ind;i++){
        NumericVector ci=Ct(_,i);
        NumericVector ri=Rt(_,i);
        double mi=sum(ci*w);
        double tmpd=sum(w*((cm-ci)*(cm-ci)+(rm-ri)*(rm-ri)*1/3.0));
        Dista(i,clu)=tmpd;
        DistM(i,clu)=(mm-mi)*(mm-mi);
